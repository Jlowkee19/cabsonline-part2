...
// Sends booking data to Node.js backend using fetch
...
export default function BookingForm() {
  ...
  const handleSubmit = async () => {
    ...
    if (!/^\d{10,12}$/.test(form.phone)) return setError('Phone must be 10-12 digits.');
    ...
    const cleanTime = form.time.replace(/\s*(AM|PM)\s*/i, '').trim();
    const timeParts = cleanTime.split(':');
    let hours   = parseInt(timeParts[0]);
    let minutes = parseInt(timeParts[1]);
    if (form.time.toUpperCase().includes('PM') && hours !== 12) hours += 12;
    if (form.time.toUpperCase().includes('AM') && hours === 12) hours = 0;
    const cleanTime24 = String(hours).padStart(2, '0') + ':' + String(minutes).padStart(2, '0');
    const pickup = new Date(parts[2], parts[1] - 1, parts[0], hours, minutes);
    if (pickup < new Date()) return setError('Pickup date and time cannot be in the past.');

    const formData = new FormData();
    Object.entries(form).forEach(([key, val]) => {
      if (key === 'time') formData.append(key, cleanTime24);
  ...
  if (confirmation) return (
    <div style={{ padding: '30px', background: '#e8f5e9', borderRadius: '8px' }}>
      <h2 style={{ color: '#2e7d32' }}>✅ Booking Confirmed!</h2>
      <p><strong>Booking Reference Number:</strong> {confirmation.brn}</p>
      <p><strong>Pickup Date:</strong> {confirmation.pdate}</p>
      <p><strong>Pickup Time:</strong> {confirmation.ptime}</p>
      <button onClick={() => setConfirmation(null)}
        style={{ marginTop: '15px', padding: '8px 20px', backgroundColor: '#1F3864',
  ...
  return (
    <div>
      ...
      {[
        ...
        <div key={name}>
          <label style={{ fontWeight: 'bold', fontSize: '14px' }}>{label}</label><br />
          <input
      ...
      <button onClick={handleSubmit}
        style={{ padding: '10px 24px', backgroundColor: '#1F3864',
